...
#[tokio::test]
async fn test_coordinator_suggestions_idempotent() {
    let (port, client) = start_server("coord-idem").await;

    // Create project with some tasks to trigger coordinator suggestions
    let resp = client
        .post(url(port, "/projects"))
        .json(&serde_json::json!({"name": "coordinator-test"}))
        .send()
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    ...
    // Create a few tasks in various states to give the coordinator something to analyze
    ...
    // First call to coordinator suggestions
    let resp = client
        .get(url(
            port,
            &format!("/projects/{}/coordinator/suggestions", project_id),
        ))
        .send()
        .await
        .unwrap();
    ...
    let count1 = body1.as_array().unwrap().len();

    // Second call — should return the same number of suggestions, not doubled
    let resp = client
        .get(url(
            port,
            &format!("/projects/{}/coordinator/suggestions", project_id),
    ...
    let count2 = body2.as_array().unwrap().len();

    assert_eq!(
        count1, count2,
        "Coordinator suggestions should be idempotent: first call returned {}, second returned {}",
        count1, count2
    );
}

// ---------------------------------------------------------------------------
// Stress test: 50+ concurrent messages
// ---------------------------------------------------------------------------
